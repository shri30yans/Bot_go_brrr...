...
class Owner(commands.Cog, name="Owner", description="Owner Commands"):
    ...
    @commands.group(
        ...
        help=f"This command is used to add Karma or Credits to users.",
    )
    ...
    async def add_credits(self, ctx, users: commands.Greedy[discord.Member], amt: int):
        ...
        embed = discord.Embed(
            title=f"Add Credit's command executed.",
            description=f"{amt} credits have been added for {given_to_users}.",
    ...
    async def add_karma(self, ctx, users: commands.Greedy[discord.Member], amt: int):
        ...
        embed = discord.Embed(
            title=f"Add Karma command executed.",
            description=f"{amt} karma have been added for {given_to_users}.",
            colour=random.choice(colour_list),
        )
        await ctx.send(embed=embed)
    ...
    @commands.group(
        ...
        invoke_without_command=True,
        case_insensitive=True,
        help=f"This command is used to set Karma or Credits to a particular number.",
    )
    ...
    @set_to.command(
        name="Credits",
        aliases=["credit", "creds", "cred"],
        help=f"Set's the credits of a user",
        require_var_positional=True,
    )  # require_var_positional=True makes sure input is not empty
    async def set_credits(self, ctx, users: commands.Greedy[discord.Member], amt: int):
                ...
                async with self.bot.pool.acquire() as connection:
                    async with connection.transaction():
                        await connection.execute(
                            "UPDATE info SET credits = $1 WHERE user_id=$2",
                            amt,
                            user.id,
                        )
                        given_to_users = given_to_users + user.mention + ", "
        ...
        embed = discord.Embed(
            title=f"Set Credits command executed.",
            description=f"{given_to_users}'s credit has been set to {amt}",
            colour=random.choice(colour_list),
        )
        await ctx.send(embed=embed)
    ...
    @set_to.command(
        name="Karma",
        aliases=["karm"],
        help=f"Set's the karma of a user",
        require_var_positional=True,
    )  # require_var_positional=True makes sure input is not empty
        ...
        for user in users:
            if user.bot:
                ...
                async with self.bot.pool.acquire() as connection:
                    async with connection.transaction():
                        await connection.execute(
                            "UPDATE info SET karma = $1 WHERE user_id=$2", amt, user.id
                        )
                        given_to_users = given_to_users + user.mention + ", "

        embed = discord.Embed(
            title=f"Set Karma command executed.",
            description=f"{given_to_users}'s karma has been set to {amt}",
            colour=random.choice(colour_list),
        )
        await ctx.send(embed=embed)
    ...
    @reset.command(
        name="All",
        aliases=["both"],
        help=f"Resets the both the Info and Starboard tables.\nFormat: `{config.default_prefixes[0]}reset all`",
    )
    async def reset_starboard(self, ctx):
        embed = discord.Embed(
            title="Reset Table confirmation",
            description="To confirm that you would like to delete the **Starboard** table type `confirm`.\nType `cancel` to cancel resetting`",
        )
        ...
        try:
            reply_message = await self.bot.wait_for(
                ...
                check=lambda m: (
                    ...
                    and ctx.content.lower() in ["confirm", "cancel", "exit"]
                ),
            )
            ...
            async with self.bot.pool.acquire() as connection:
                async with connection.transaction():
                    ...
                    embed = discord.Embed(
                        title="Starboard and Info Table resetted.",
                        description=f"Both the tables have resetted.",
                        colour=random.choice(colour_list),
                    )
                    await ctx.send(embed=embed)
    ...
    @commands.is_owner()
    @reset.command(
        name="Starboard", aliases=["sb"], help=f"Resets the both the Starboard table."
    )
        ...
        try:
            ...
            async with self.bot.pool.acquire() as connection:
                async with connection.transaction():
                    await connection.execute("DELETE FROM info")
                    embed = discord.Embed(
                        title="Info Table resetted.",
                        description=f"The entire Info table has been resetted.",
                        colour=random.choice(colour_list),
                    )
                    await ctx.send(embed=embed)
    ...
    async def reset_all_polls(self, ctx):
        async with self.bot.pool.acquire() as connection:
            async with connection.transaction():
                ...
                embed = discord.Embed(
                    title="All Poll's have been resetted.",
                    description=f"Any ongoing poll's will no longer work.",
                    colour=random.choice(colour_list),
                )
                await ctx.send(embed=embed)
    ...
    async def reset_karma(self, ctx, amt=0):
        async with self.bot.pool.acquire() as connection:
            async with connection.transaction():
                ...
                embed = discord.Embed(
                    title="Set Karma command executed.",
                    description=f"Karma for all users has been set to {amt}",
                    colour=random.choice(colour_list),
                )
                await ctx.send(embed=embed)
    ...
    @reset.command(
        ...
        help=f"Resets the credits for all users.",
    )
    async def reset_credits(self, ctx, amt=0):
        async with self.bot.pool.acquire() as connection:
            async with connection.transaction():
                ...
                embed = discord.Embed(
                    title="Set Credits command executed.",
                    description=f"Credits for all users has been set to {amt}",
                    colour=random.choice(colour_list),
                )
                await ctx.send(embed=embed)
    ...
    @commands.command(
        name="Leave",
        help=f"Leave a guild. Leaves the current guild if no guild id is mentioned",
    )
    async def leave(self, ctx, guild: discord.Guild = None):
        guild = guild or ctx.guild
        user = ctx.author
        embed = discord.Embed(
            title=f"Leave command executed.",
    ...
    @commands.command(
        name="Nickname",
        aliases=["nick"],
        help=f"Change the bot nickname in that server.",
    )
    async def change_nickname(self, ctx, new_nickname: str):
        user = ctx.author
        embed = discord.Embed(
            title=f"Nickname command executed.",
            description=f"{self.bot.user.mentions}'s nickname has been changed to {new_nickname}",
            colour=random.choice(colour_list),
        )
        embed.set_thumbnail(url=str(ctx.guild.icon_url))
        embed.set_footer(

...
class Events(commands.Cog):
    ...
    async def startup(self):
        if config.run_event:  # Boolean
            ...
            while True:
                ...
                time_interval = random.randint(
                    least_delay * 60, max_delay * 60
                )  # converts to seconds
                # time_interval=60
    ...
    async def event(self, ctx, rarity="Random"):
        ...
        if rarity == "Random":
            ...
            await ctx.send(
                "Enter a proper rarity. (Common, Uncommon, Rare, Epic, Legendary)."
            )

        if rarity == "Common":
            questions = {
                "You are really poor. Quick! Beg for some credits. Type `?beg`.": [
                    "?beg"
                ],
                "Someone asked you for some credits. Quick! Type `no`": ["no"],
                "Someone just roasted you. Quick! Type `Your mom` ": [
                    "your mom",
                ],
            ...
            try:
                msg = await self.bot.wait_for(
                    "message",
                    check=lambda m: (m.content.lower() in questions[word]),
                    timeout=300,
                ...
                await ctx.send(
                    f"{user.mention} wins {amt} credits for answering first!"
                )
                await sent_msg.edit(
            ...
            try:
                msg = await self.bot.wait_for(
                    "message",
                    check=lambda m: (m.content.lower() == questions[word].lower()),
                    timeout=180.0,
                )
                ...
                await ctx.send(
                    f"{user.mention} wins {amt} credits! The missing word was `{questions[word]}`"
                )
                await sent_msg.edit(
                    content=f"{sent_msg.content}\n\n This event is expired. No new submissions will be accepted"
                )
            ...
            try:
                msg = await self.bot.wait_for(
                    "message",
                    check=lambda m: (m.content.lower() == word.lower()),
                    timeout=180.0,
                )
                ...
                await ctx.send(
                    f"{user.mention} wins {amt} credits! The word was `{word}`"
                )
                await sent_msg.edit(
                    content=f"{sent_msg.content}\n\n This event is expired. No new submissions will be accepted"
                )
            ...
            questions = {
                "What is the name of the Internet meme of a dinosaur thinking?": [
                    "philosoraptor",
                ],
                "What is the name of the anti-social and highly-territorial green ogre who loves the solitude of his swamp?": [
                    "shrek"
                ],
                "What is the name of this small 50 year old with long ears beloging to a popular franchise that took the internet by storm in 2019?": [
                    "grogu"
                ],
                "What is the name of the humorous pink starfish, who serves as the village idiot in an underwater city.": [
                    "patrick"
                ],
            }
            word = random.choice(list(questions))
            ...
            try:
                ...
                await ctx.send(
                    f"{user.mention} wins {amt} credits! The answer was `{questions[word][0].capitalize()}`"
                )
                await sent_msg.edit(
                    content=f"{sent_msg.content}\n\n This event is expired. No new submissions will be accepted"
                )
            ...
            questions = {
                "https://i.imgur.com/HBUtYuZ.jpg": ["this is fine", "this is fine."],
                "https://i.imgur.com/IyX19td.png": ["stonks"],
                "https://i.imgur.com/2B85WEe.png": [
                    "aight imma head out",
                    "ight imma head out",

...
class Settings(commands.Cog, name="Settings", description="Bot settings"):
    ...
    @commands.command(
        name="ChangePrefix",
        aliases=["newprefix", "updateprefix", "prefixchange"],
        help=f"Change the bot prefix for this server",
    )
    async def change_prefix(self, ctx, new_prefix: str):
        ...
        if new_prefix in prefix_list:
            ...
            embed = discord.Embed(
                ...
                description=f"New server prefix has been updated to `{new_prefix}`",
                colour=random.choice(colour_list),
            )
            await ctx.send(embed=embed)
            ImportantFunctions = self.bot.get_cog("ImportantFunctions")
            await ImportantFunctions.update_server_prefix(
                new_prefix=new_prefix, guild_id=guild_id
            )

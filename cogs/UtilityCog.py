...
class Utility(commands.Cog, name="Utility", description="Utilty functions"):
    ...
    @commands.command(
        name="Prefix", aliases=["prefixes"], help=f"Get the server prefix"
    )
    async def show_prefix(self, ctx):
        ...
        embed = discord.Embed(
            title="Prefix",
            description="A new prefix can be set by using the command `newprefix`",
    ...
    async def ping(self, ctx):
        ...
        message = await ctx.reply(
            embed=discord.Embed(
                title=f"{config.loading_reaction}  Pinging",
                description="Pinging...",
                color=random.choice(colourlist),
                timestamp=ctx.message.created_at,
            )
        ...
        async with self.bot.pool.acquire() as connection:
            async with connection.transaction():
                await connection.fetchrow(
                    "SELECT starboard FROM server_info WHERE id=$1", ctx.guild.id
                )
        end = time.perf_counter()
        ...
        embed.add_field(
            name="Database", value=f"```{round(database_ping)}ms```", inline=True
        )
        embed.set_footer(
            icon_url=ctx.author.avatar_url,
            text=f"Requested by {ctx.message.author} • {self.bot.user.name} ",
    ...
    @commands.command(
        name="Avatar", aliases=["dp", "pfp", "av"], help=f"Shows the avatar of a user"
    )
    ...
    @commands.cooldown(1, 5, commands.BucketType.user)
    @commands.command(
        name="Notifications", aliases=["notifs", "logs"], help=f"See your notifications"
    )
    async def notifications(self, ctx, page=1):
        ...
        embed = discord.Embed(
            title=f"{user.name} notifications",
            color=random.choice(colourlist),
            timestamp=ctx.message.created_at,
        )
        for log in logs:
            ...
            if log["type"] == "robbed":
                ...
                embed.add_field(
                    name="You were robbed from!",
                    value=f"{user_related.mention} has stolen **{log['amount']}** credits from you in **{guild.name}**!",
                )
                ...
                guild = self.bot.get_guild(log["guild_id"])
                embed.add_field(
                    name="You were given credits!",
                    value=f"{user_related.mention} has given you **{log['amount']}** credits in **{guild.name}**!",
                )
                ...
                if guild is None:
                    embed.add_field(
                        name="You were given a award!",
                        value=f"{user_related} has given you a **{award.name}** Award.",
                    )
                elif None in [channel, message]:
                    embed.add_field(
                        name="You were given a award!",
                        value=f"{user_related} has given you a **{award.name}** Award in **{guild.name}**!",
                    )
                else:
                    embed.add_field(
                        name="You were given a award!",
                        value=f'{user_related} has given you a **{award.name}** Award for this [message]({message.jump_url} "See the Awarded message.") in **{guild.name}**!',
                    )
        if len(logs) == 0:
            embed.add_field(
                name="No notifications", value=f"You have no notifcations to show."
            )
    ...
    async def poll(self, ctx, *arguments):
        ...
        if result is None:
            await ctx.send(
                "You didn't mention a question. Format: `[Question] Option_1, Option_2, Option_3`"
            )
            return
        ...
        if len(options) > 10:
            await ctx.send(
                "You inputted too many options. Maximum number of options is 10."
            )

        elif len(options) <= 1:
            await ctx.send(
                "You inputted too less options. Minimum number of options is 2."
            )
        ...
        # Proper outcome. There is a question and 1-10 Options
            ...
            for option in options:
                ...
                embed.add_field(
                    name=f"{number_emojis[optionnum]} {option.lstrip().capitalize()}",
                    value=f"`{bar_string}` | {percent}% | (0)",
                    inline=False,
                )
                optionnum += 1

            embed.set_footer(
                text=f"You may choose only one option • {self.bot.user.name} "
            )
            ...
            async with self.bot.pool.acquire() as connection:
                async with connection.transaction():
                    ...
                    server_info = await connection.fetchrow(
                        "SELECT * FROM server_info WHERE id=$1", ctx.guild.id
                    )
                    ...
                    await connection.execute(
                        "UPDATE server_info SET ongoing_polls = $1 WHERE id=$2",
                        ongoing_polls_j,
                        ctx.guild.id,
                    )
    ...
    async def update_poll(
        self, PollMessage, emoji, user, type_of_event
    ):  # type of event = reaction_add, reaction_remove
        # print("update",str(emoji))
        if (
            str(emoji) == "\U0000274c" and type_of_event == "reaction_add"
                ...
                async with self.bot.pool.acquire() as connection:
                    async with connection.transaction():
                        server_info = await connection.fetchrow(
                            ...
                            PollMessage.guild.id,
                        )
                        ongoing_polls = json.loads(
                            server_info["ongoing_polls"]
                        )  # load the json content of  ongoing_polls
                        ongoing_polls_list = ongoing_polls[
                            "polls"
                        ]  # fetch the list of all ids
                        ongoing_polls_list.remove(PollMessage.id)  # remove that id
                        ongoing_polls_j = json.dumps(
                            ongoing_polls
                        )  # change back to json
                        await connection.execute(
                            "UPDATE server_info SET ongoing_polls = $1 WHERE id=$2",
            ...
            async def update_poll_bar():
                ...
                for x in embed_fields:
                    ...
                    bar_string, percent = self.bar_generator(
                        count=count, total=total_reactions
                    )
                    embed.set_field_at(
                        embed_fields.index(x),
                        name=f"{x.name}",
                        value=f"`{bar_string}` | {percent}% | ({count})",
                        inline=False,
                    )
                await PollMessage.edit(embed=embed)
    ...
    async def text_input_function(self, ctx, title: str, text: str):
        ...
        try:
            ...
            await question_embed.edit(
                embed=discord.Embed(
                    title="Timeout Error",
                    description="You took too much time, ya retarded monkey",
                    color=random.choice(colourlist),
                    timestamp=ctx.message.created_at,
                ).set_footer(
                    icon_url=ctx.author.avatar_url,
            ...
            if len(text.content) > 50:
                embed = discord.Embed(
                    title="<:warn:789487083802460200> | Too many Characters ",
                    color=random.choice(colourlist),
                )
                embed.add_field(
                    name="Type only 50 characters in your sentence.",
                    value=f"You typed {len(text.content)} letters. Type the text again.",
                    inline=False,
                )
                embed.set_footer(
                    icon_url=ctx.author.avatar_url,

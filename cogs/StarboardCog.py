...
class Starboard(commands.Cog,name="Starboard",description="Starboard functions"): 
    ...
    @starboard.command(name="Settings",aliases=["setting","changesettings"],help=f"Change starboard settings")
    async def settings(self,ctx):
        ...
        async def wait_for_reactions(message,valid_reactions_list):
            def check(reaction,user):
                return (str(reaction.emoji) in valid_reactions_list) and (user == ctx.author)
        ...
        async def channel_settings(message):
            ...
            await message.clear_reactions()
            embed = discord.Embed(title="Starboard Channel Settings",description=f"Select an option.",color = random.choice(colourlist),timestamp=ctx.message.created_at)
            embed.add_field(name="Starboard Channel",value = f"Current channel: {starboard_channel.mention}\nIf you would like to edit this setting choose `✏️`\nSelect `❌` to return back to home.")     
            embed.set_footer(icon_url= ctx.author.avatar_url,text=f"Requested by {ctx.message.author} • {self.bot.user.name}")
            await message.edit(embed = embed) 
            reactions_list=["✏️","❌"]
            ...
            if emoji == "❌":
                ...
                await message.clear_reactions()
                embed = discord.Embed(title="Enter the new starboard channel.",description=f"Current starboard channel: {starboard_channel.mention} \nMention the channel or type it's ID.\nCancel setup command by typing `cancel`.",color = random.choice(colourlist),timestamp=ctx.message.created_at)
                embed.add_field(name="Important note:",value="Proceeding with this action will **delete** all post information regarding old starboard posts. Any old starred posts will no longer get updated.")
                embed.set_footer(icon_url= ctx.author.avatar_url,text=f"Requested by {ctx.message.author} • {self.bot.user.name}")
                await message.edit(embed = embed)
                ...
                async def ask_channel_name(ctx,message,retries):
                    try:
                        responsemessage = await self.bot.wait_for('message', check=lambda m:(m.author == ctx.author), timeout=120)
                        ...
                        if responsemessage.content.lower() in ["cancel","exit"]:
                            ...
                            try:
                                channel= await commands.TextChannelConverter().convert(ctx,responsemessage.content)
                                if channel == starboard_channel:
                                    if retries > 0:
                                        embed = discord.Embed(title="Invalid channel entered",description=f"{channel.mention} is already set as the starboard channel.\nMention another channel or type it's ID.\nCancel setup command by typing `cancel`.\nRetries left: `{retries}`",color = random.choice(colourlist),timestamp=ctx.message.created_at)
                                        embed.set_footer(icon_url= ctx.author.avatar_url,text=f"Requested by {ctx.message.author} • {self.bot.user.name}")
                                        await message.edit(embed = embed)
                                        retries -= 1
                                        return (await ask_channel_name(ctx,message,retries))
        ...
        async def star_limit_settings(message):
                ...
                await message.clear_reactions()
                embed = discord.Embed(title="Starboard Limit Settings",description=f"Select an option.",color = random.choice(colourlist),timestamp=ctx.message.created_at)
                embed.add_field(name="Stars required",value = f"Current stars requirement: `{star_limit}`\nIf you would like to edit this setting choose `✏️`\nSelect `❌` to return back to home.")     
                embed.set_footer(icon_url= ctx.author.avatar_url,text=f"Requested by {ctx.message.author} • {self.bot.user.name}")
                await message.edit(embed = embed) 
                reactions_list=["✏️","❌"]
                await add_reactions_to_submenu(message,reactions_list)   
                ...
                if emoji == "❌":
                    ...
                    await message.clear_reactions()
                    embed = discord.Embed(title="Enter the new stars required ",description=f"Current stars required: `{star_limit}` \nEnter the new stars required\nCancel setup command by typing `cancel`.",color = random.choice(colourlist),timestamp=ctx.message.created_at)
                    embed.set_footer(icon_url= ctx.author.avatar_url,text=f"Requested by {ctx.message.author} • {self.bot.user.name}")
                    await message.edit(embed = embed)
                            ...
                            if responsemessage.content.lower() in ["cancel","exit"]:
                                ...
                                try:
                                    ...
                                    if new_star_limit == star_limit:
                                        if retries > 0:
                                            embed = discord.Embed(title="Invalid limit entered",description=f"`{star_limit}` is already set as the limit.\nMention another limit.\nCancel setup command by typing `cancel`.\nRetries left: `{retries}`",color = random.choice(colourlist),timestamp=ctx.message.created_at)
                                            embed.set_footer(icon_url= ctx.author.avatar_url,text=f"Requested by {ctx.message.author} • {self.bot.user.name}")
                                            await message.edit(embed = embed)
                                            retries -= 1
                                    ...
                                    if retries > 0:
                                        embed = discord.Embed(title="Invalid limit entered",description=f"The Star limit needs to be a positive integer between 1 to 200.\nCancel setup command by typing `cancel`.\nRetries left: `{retries}`",color = random.choice(colourlist),timestamp=ctx.message.created_at)
                                        embed.set_footer(icon_url= ctx.author.avatar_url,text=f"Requested by {ctx.message.author} • {self.bot.user.name}")
                                        await message.edit(embed = embed)
                                        retries -= 1
                                        return (await ask_star_limit(ctx,message,retries))
        ...
        async def self_star_settings(message):
                ...
                await message.clear_reactions()
                embed = discord.Embed(title="Starboard self star Settings",description=f"Select an option.",color = random.choice(colourlist),timestamp=ctx.message.created_at)
                embed.add_field(name="Self star",value = f"Current self star setting: `{self_star}`\nIf you would like to edit this setting choose `✏️`\nSelect `❌` to return back to home.")     
                embed.set_footer(icon_url= ctx.author.avatar_url,text=f"Requested by {ctx.message.author} • {self.bot.user.name}")
                await message.edit(embed = embed) 
                reactions_list=["✏️","❌"]
                await add_reactions_to_submenu(message,reactions_list)   
                ...
                if emoji == "❌":
                    ...
                    await message.clear_reactions()
                    embed = discord.Embed(title="Enter the self star settings",description=f"Current self star setting: `{self_star}` \nType `{not(self_star)}` to change it.\nCancel setup command by typing `cancel`.",color = random.choice(colourlist),timestamp=ctx.message.created_at)
                    embed.set_footer(icon_url= ctx.author.avatar_url,text=f"Requested by {ctx.message.author} • {self.bot.user.name}")
                    await message.edit(embed = embed)
                        ...
                        try:
                            ...
                            if responsemessage.content.lower() in ["cancel","exit"]:
                                ...
                                if check is self_star:
                                    if retries > 0:
                                        embed = discord.Embed(title="Invalid value entered",description=f"Current self star limit is already {self_star}.\nType `{not(self_star)}` to change it.\nCancel setup command by typing `cancel`.\nRetries left: `{retries}`",color = random.choice(colourlist),timestamp=ctx.message.created_at)
                                        embed.set_footer(icon_url= ctx.author.avatar_url,text=f"Requested by {ctx.message.author} • {self.bot.user.name}")
                                        await message.edit(embed = embed)
                                        retries -= 1
                                    ...
                                    if retries > 0:
                                        embed = discord.Embed(title="Invalid value entered",description=f"Self star can only be set to `True` or `False`\nCancel setup command by typing `cancel`.\nRetries left: `{retries}`",color = random.choice(colourlist),timestamp=ctx.message.created_at)
                                        embed.set_footer(icon_url= ctx.author.avatar_url,text=f"Requested by {ctx.message.author} • {self.bot.user.name}")
                                        await message.edit(embed = embed)
                                        retries -= 1
                                        return (await ask_self_star(ctx,message,retries))
        ...
        async def nsfw_channel_settings(message):
                ...
                await message.clear_reactions()
                embed = discord.Embed(title="Starboard NSFW channel Settings",description=f"Select an option.",color = random.choice(colourlist),timestamp=ctx.message.created_at)
                embed.add_field(name="NSFW channel",value = f"Current NSFW setting: `{nsfw}`\nIf you would like to edit this setting choose `✏️`\nSelect `❌` to return back to home.")     
                embed.set_footer(icon_url= ctx.author.avatar_url,text=f"Requested by {ctx.message.author} • {self.bot.user.name}")
                await message.edit(embed = embed) 
                reactions_list=["✏️","❌"]
                await add_reactions_to_submenu(message,reactions_list)   
                ...
                if emoji == "❌":
                    ...
                    await message.clear_reactions()
                    embed = discord.Embed(title="Enter the NSFW channel settings",description=f"Current NSFW channel setting: `{nsfw}` \nType `{not(nsfw)}` to change it.\nCancel setup command by typing `cancel`.",color = random.choice(colourlist),timestamp=ctx.message.created_at)
                    embed.set_footer(icon_url= ctx.author.avatar_url,text=f"Requested by {ctx.message.author} • {self.bot.user.name}")
                    await message.edit(embed = embed)
                    ...
                    async def ask_nsfw_settings(ctx,message,retries):
                        try:
                            ...
                            if responsemessage.content.lower() in ["cancel","exit"]:
                                ...
                                if check is nsfw:
                                        if retries > 0:
                                            embed = discord.Embed(title="Invalid value entered",description=f"Current NSFW channel setting is already {nsfw}.\nType `{not(nsfw)}` to change it.\nCancel setup command by typing `cancel`.\nRetries left: `{retries}`",color = random.choice(colourlist),timestamp=ctx.message.created_at)
                                            embed.set_footer(icon_url= ctx.author.avatar_url,text=f"Requested by {ctx.message.author} • {self.bot.user.name}")
                                            await message.edit(embed = embed)
                                            retries -= 1
                                            return (await ask_nsfw_settings(ctx,message,retries))
                                        else:
                                            await exit_command(message=message,user=ctx.author,reason=f"{ctx.author.mention} reached the maximum number of retries.")
                                            return
                                    ...
                                    if retries > 0:
                                        embed = discord.Embed(title="Invalid value entered",description=f"NSFW channel settings can only be set to `True` or `False`\nCancel setup command by typing `cancel`.\nRetries left: `{retries}`",color = random.choice(colourlist),timestamp=ctx.message.created_at)
                                        embed.set_footer(icon_url= ctx.author.avatar_url,text=f"Requested by {ctx.message.author} • {self.bot.user.name}")
                                        await message.edit(embed = embed)
                                        retries -= 1
                                        return (await ask_nsfw_settings(ctx,message,retries))
        ...
        async def private_channel_settings(message):
                ...
                await message.clear_reactions()
                embed = discord.Embed(title="Starboard Private channel Settings",description=f"Select an option.",color = random.choice(colourlist),timestamp=ctx.message.created_at)
                embed.add_field(name="Private channel",value = f"Current private channel setting: `{private}`\nIf you would like to edit this setting choose `✏️`\nSelect `❌` to return back to home.")     
                embed.set_footer(icon_url= ctx.author.avatar_url,text=f"Requested by {ctx.message.author} • {self.bot.user.name}")
                await message.edit(embed = embed) 
                reactions_list=["✏️","❌"]
                await add_reactions_to_submenu(message,reactions_list)   
                ...
                if emoji == "❌":
                    ...
                    await message.clear_reactions()
                    embed = discord.Embed(title="Enter the private channel settings",description=f"Current Private channel setting: `{private}` \nType `{not(private)}` to change it.\nCancel setup command by typing `cancel`.",color = random.choice(colourlist),timestamp=ctx.message.created_at)
                    embed.set_footer(icon_url= ctx.author.avatar_url,text=f"Requested by {ctx.message.author} • {self.bot.user.name}")
                    await message.edit(embed = embed)
                    ...
                    async def ask_private_settings(ctx,message,retries):
                        try:
                            ...
                            if responsemessage.content.lower() in ["cancel","exit"]:
                                ...
                                if check is private:
                                    if retries > 0:
                                        embed = discord.Embed(title="Invalid value entered",description=f"Current Private channel setting is already {private}.\nType `{not(private)}` to change it.\nCancel setup command by typing `cancel`.\nRetries left: `{retries}`",color = random.choice(colourlist),timestamp=ctx.message.created_at)
                                        embed.set_footer(icon_url= ctx.author.avatar_url,text=f"Requested by {ctx.message.author} • {self.bot.user.name}")
                                        await message.edit(embed = embed)
                                        retries -= 1
                                        return (await ask_private_settings(ctx,message,retries))
                                    else:
                                        await exit_command(message=message,user=ctx.author,reason=f"{ctx.author.mention} reached the maximum number of retries.")
                                        return
                                    ...
                                    if retries > 0:
                                        embed = discord.Embed(title="Invalid value entered",description=f"Private channel settings can only be set to `True` or `False`\nCancel setup command by typing `cancel`.\nRetries left: `{retries}`",color = random.choice(colourlist),timestamp=ctx.message.created_at)
                                        embed.set_footer(icon_url= ctx.author.avatar_url,text=f"Requested by {ctx.message.author} • {self.bot.user.name}")
                                        await message.edit(embed = embed)
                                        retries -= 1
                                        return (await ask_private_settings(ctx,message,retries))
                    ...
                    new_private = await ask_private_settings(ctx,message=message,retries=3)
        ...
        async def settings_menu(message = None):
            ...
            embed = discord.Embed(title="Starboard Settings",description=f"Select an option.",color = random.choice(colourlist),timestamp=ctx.message.created_at)
            embed.add_field(name="`1️⃣` Channel",value = f"Change channel settings\nCurrent channel: {starboard_channel.mention}",inline = False)    
            embed.add_field(name="`2️⃣` Stars required",value = f"Change stars required settings\nCurrent stars required: `{star_limit}`",inline = False)  
            embed.add_field(name="`3️⃣` Self star",value = f"Toggle users able to star their own posts\nCurrent setting: `{self_star}`",inline = False)  
            embed.add_field(name="`4️⃣` NSFW Channel settings",value = f"Change ability to post from NSFW channels.\nCurrent setting: `{nsfw}`",inline = False)  
            embed.add_field(name="`5️⃣` Private Channel settings",value = f"Change ability to post from private channels.\nCurrent setting: `{private_channel}`",inline = False) 
            embed.add_field(name="`❌` Exit",value = f"Slect the `❌` reaction to exit the menu.",inline = False) 
            embed.set_footer(icon_url= ctx.author.avatar_url,text=f"Requested by {ctx.message.author} • {self.bot.user.name}")

...
class ReactionCog(commands.Cog):
    ...
    async def upvote_downvote_reaction_add(self, payload):
        ...
        if (
            channel.guild.id in config.APPROVED_SERVERS
        ):  # if that server is approved/that server has the settings

            await self.check_if_message_is_poll(
                message=message, emoji=emoji, user=user, type_of_event="reaction_add"
            )
            ...
            if (
                str(emoji) == config.upvote_reaction
                and message.channel.id == config.meme_channel_id
            ):
    ...
    async def upvote_downvote_reaction_remove(self, payload):
        ...
        if (
            ...
            await self.check_if_message_is_poll(
                message=message, emoji=emoji, user=user, type_of_event="reaction_remove"
            )
    ...
    async def send_to_dank_memes_channel(self, message):
        async def send_message():
            embed = discord.Embed(
                color=meme_channel.guild.me.colour,
                ...
                if count >= 3:
                    ...
                    if str(user.id) in self.messages_count_dict:
                        if self.messages_count_dict[str(user.id)] >= 5:
                            ...
                            # Add credits
                            UserDatabaseFunctions = self.bot.get_cog(
        ...
        async with self.bot.pool.acquire() as connection:
            async with connection.transaction():
                server_info = await connection.fetchrow(
                    "SELECT * FROM server_info WHERE id=$1", message.guild.id
                )
                ongoing_polls = json.loads(server_info["ongoing_polls"])
                ongoing_polls_list = ongoing_polls["polls"]

                if (
                    message.id in ongoing_polls_list
                ):  # if the message_id is in ongoing_polls
                    UtilityCog = self.bot.get_cog("Utility")
                    await UtilityCog.update_poll(
                        PollMessage=message,
                        emoji=emoji,
                        user=user,

...
class Economy(commands.Cog, name="Economy", description="Economy functions"):
    ...
    @commands.command(
        name="Stats",
        aliases=["account", "bal", "acc", "balance", "karma", "credits", "profile"],
        help=f"Shows the Karma, Credits and Awards of a user",
    )
    async def bal(self, ctx, user: discord.Member = None):
        user = user or ctx.author
        sent_msg = await ctx.reply(
            embed=discord.Embed(
                title=f"{config.loading_reaction}    {user.name}'s Balance",
                description=f"Fetching {user.name}'s inventory from the database...",
        ...
        if user.bot:
            ...
            embed = discord.Embed(
                title=f"{user.name}'s Balance", colour=random.choice(colourlist)
            )
            badges = await self.format_in_order(
                to_format=json.loads(user_account["badges"])["badges"], type="badge"
            )
            embed.add_field(name="Badges:", value=f"{badges}", inline=False)
            embed.add_field(
                name="Balance:",
                value=f"{'{:,}'.format(user_account['credits'])} Credits",
                inline=True,
            )
            embed.add_field(
                name="Karma:",
                value=f"{'{:,}'.format(user_account['karma'])} Karma",
                inline=True,
            )

            awards_given, total_awards_given = await self.format_in_order(
                to_format=json.loads(user_account["awards_given"]), type="award"
            )
            awards_received, total_awards_received = await self.format_in_order(
                to_format=json.loads(user_account["awards_received"]), type="award"
            )
            ...
            embed.add_field(
                name="Awards given:",
                value=f"{awards_given}\nTotal awards given: {total_awards_given}",
                inline=False,
            )
            embed.add_field(
                name="Awards received:",
                value=f"{awards_received}\nTotal awards received: {total_awards_received}",
                inline=False,
            )
    ...
    @commands.command(
        name="Awards",
        aliases=["awardlist"],
        help=f"Shows list of all the awards with their description, cost and other details",
    )
    async def award_list(self, ctx):
        ...
        embed = discord.Embed(
            title=f"Awards",
            description=f"Your balance: **{credits} Credits**",
            colour=random.choice(colourlist),
        )

        for award in list(awards.awards_list.values()):
            embed.add_field(
                name=f"{award.reaction_id} {award.name} ",
                value=f"{'{:,}'.format(award.cost)} credits \n {award.description} \n",
                inline=False,
            )
    ...
    @commands.command(
        name="Badges",
        aliases=["badge,badgelist"],
        help=f"Shows list of all the badges with their description, cost and other details.",
    )
    async def badge_list(self, ctx):
        ...
        credits = await UserDatabaseFunctions.get_user_credits(user)
        embed = discord.Embed(
            title=f"Badges", description=f"Your balance: **{credits} Credits**"
        )

        embed.add_field(
            name="**Credits based**",
            value="**These badges can be bought in exchange of credits.**",
        )
        for badge in list(badges.badges_list.values()):
            if badge.cost is not None:
                embed.add_field(
                    name=f"{badge.reaction_id} {badge.name} ",
                    value=f"{'{:,}'.format(badge.cost)} credits \n {badge.description} \n",
                    inline=False,
                )

        embed.add_field(
            name="**Karma based**",
            value="**These badges are earned when you reach a certain karma.**",
        )
        for badge in list(badges.badges_list.values()):
            if badge.karma_required is not None:
                embed.add_field(
                    name=f"{badge.reaction_id} {badge.name} ",
                    value=f"{badge.karma_required} karma \n {badge.description} \n",
                    inline=False,
                )

        embed.add_field(
            name="**Special**",
            value="**These badges can only be earned through special methods.**",
        )
        for badge in list(badges.badges_list.values()):
    ...
    async def award(self, ctx, message: discord.Message = None):
        ...
        try:
            ...
            if ctx.message.reference is None and message is None:
                ...
                embed.add_field(
                    name="You didn't mention a message.",
                    value="To award a post:\n   - React with `🏆` to a message\n   - Use this command as a reply to the post you would like to award",
                )
                await ctx.channel.send(
                    content=user_giving.mention, embed=embed, delete_after=5
                )
                return
        ...
        if user_recieving.bot:
            embed = discord.Embed(
                title="Award",
                description=f"{user_giving.mention} you can't award bots.",
                colour=random.choice(colourlist),
            )
            await ctx.channel.send(
                content=user_giving.mention, embed=embed, delete_after=5
            ...
            embed = discord.Embed(
                title="Award",
                description=f"{user_giving.mention} you can't award yourself.",
                colour=random.choice(colourlist),
            )
            await ctx.channel.send(
                content=user_giving.mention, embed=embed, delete_after=5
        ...
        embed.add_field(
            name=f"{user_giving.name} react with the award you would like to give {user_recieving.name}",
            value=f"An award **cannot** be revoked once given. This action is irreversible.\nChoose a reaction if you would like to award this [post]({message.jump_url}).\n Select `❌` if you would like to cancel the award.",
        )
        ...
        def check_if_award(award_reaction, award_user):
            return (
                str(award_reaction.emoji) in award_reaction_ids
                or str(award_reaction.emoji) in ["❌"]
            ) and user_giving == award_user

        try:
            award_reaction, award_user = await self.bot.wait_for(
                "reaction_add", check=check_if_award, timeout=60
            ...
            if str(award_reaction) in ["❌"]:
                ...
                embed = discord.Embed(
                    title=f"{message.author.name} received a {award.name} Award!",
                    description=f"{user_giving.mention} liked {message.author.mention}'s [post]({message.jump_url}) so much that they gave it the {award.name} award.",
                    color=0xFFD700,
                )
                embed.set_thumbnail(url=str(award_reaction.emoji.url))
                embed.set_footer(
                    icon_url=user_giving.avatar_url,
                    text=f"Given by {user_giving.name} • {self.bot.user.name} ",
                )
            ...
            async def add_award(user_recieving, user_giving, award):
                await UserDatabaseFunctions.add_karma(
                    user=user_recieving, amt=award.karma_given_to_receiver
                )
                await UserDatabaseFunctions.add_karma(
                    user=user_giving, amt=award.karma_given_to_giver
                )

                await UserDatabaseFunctions.add_credits(
                    user=user_giving, amt=-award.cost
                )
                await UserDatabaseFunctions.add_credits(
                    user=user_recieving, amt=award.credits_given_to_receiver
                )

                await UserDatabaseFunctions.add_awards(
                    user_recieving=user_recieving,
                    user_giving=user_giving,
                    award_name=award.name,
                )

                ImportantFunctions = self.bot.get_cog("ImportantFunctions")
                await ImportantFunctions.update_logs(
                    type="award",
                    user_id=user_recieving.id,
                    related_user_id=user_giving.id,
        ...
        if item:
            if item.cost is not None:
                ...
                if credits < item.cost:
                    await ctx.reply(
                        f"You don't have enough credits to buy `{item.name}`."
                    )
                else:
                    await UserDatabaseFunctions.add_credits(user, -item.cost)
                    check = await UserDatabaseFunctions.edit_badges(
                        badge_name=item.name, user=user, action="add"
                    )
                    if check:
                        await ctx.reply(
                            content=f"You paid {item.cost} credits and bought `{item.name}`."
                        )
                    else:
                        await ctx.reply(
                            content=f"You already have the item `{item.name}`."
                        )
            else:
    ...
    async def beg(self, ctx):
        ...
        if choice == "credits":
            ...
            beg_options = [
                f"Someone gave you {amt} credits.",
                f"Awh you begging? Take {amt} credits.",
                f"You begged and got {amt} credits.",
                f"Lmao sad guy. Take {amt} credits.",
            ]
            ...
            beg_options = [
                f"no",
                f'*"Sorry no money for you."*',
                f"imagine begging lol",
                "Even your comrades said no",
                "Nah I gave away all my money",
                "credit.exe cannot afford your begging",
                "donate.exe has stopped working",
                "I ain't made of money. go away",
                "You know how people say they got your back? Well I don't. Go away.",
            ]
            await ctx.reply(random.choice(beg_options))
    ...
    async def rob(self, ctx, user_robbed: discord.Member):
        ...
        if user_robbed.bot:
            ...
            await ctx.reply(
                "Robbing yourself? huh you must be a some different kind of nuts."
            )
            ctx.command.reset_cooldown(ctx)

        elif await UserDatabaseFunctions.get_user_passive_mode(user_robbing):
            await ctx.send(
                "You are in passive mode. Turn that off before sharing dum dum."
            )

        elif await UserDatabaseFunctions.get_user_passive_mode(user_robbed):
            await ctx.send(
                f"{user_robbed.name} is in passive mode. You can't share with them."
            )

        else:

            user_robbing_credits = await UserDatabaseFunctions.get_user_credits(
                user=user_robbing
            )
            user_robbed_credits = await UserDatabaseFunctions.get_user_credits(
                user=user_robbed
            )
            ...
            if user_robbing_credits < user_robbing_credits_required:
                await ctx.reply(
                    f"You need at least {user_robbing_credits_required} credits to rob others."
                )
                ctx.command.reset_cooldown(ctx)

            elif user_robbed_credits < user_robbed_credits_required:
                await ctx.reply(
                    f"Leave the poor folk alone. They have less than {user_robbed_credits_required} credits. sad guy"
                )
                ctx.command.reset_cooldown(ctx)
            else:
                has_not_been_robbed_in_last_10_seconds = (
                    await checks.get_last_robbed_from(ctx, user=user_robbed, delay=600)
                )
                if has_not_been_robbed_in_last_10_seconds:
                    ...
                    choice = random.choices(
                        options, weights=(30, 20, 30, 15, 10, 5, 1), k=1
                    )[0]
                    if choice == "fail":
                        amt = int(random.randint(100, 300))
                        await UserDatabaseFunctions.give(
                            user_giving=user_robbing, user_taking=user_robbed, amt=amt
                        )
                        await ctx.reply(
                            f"You failed lmao. You lost {amt} credits to {user_robbed.name}."
                        )
                        ...
                        await ctx.reply(
                            f"You got caught lmao. You paid {user_robbed.name} **{amt}** credits. ded xd"
                        )
                        ...
                        amt = int(percentage * user_robbing_credits)
                        await UserDatabaseFunctions.give(
                            user_giving=user_robbed, user_taking=user_robbing, amt=amt
                        )

                    elif choice == "loot":
                        percentage = (
                            random.choice(list(range(2, 3)) + list(range(4, 5))) / 100
                        )
                        amt = int(percentage * user_robbing_credits)
                        await UserDatabaseFunctions.give(
                            user_giving=user_robbed, user_taking=user_robbing, amt=amt
                        ...
                        await ctx.reply(
                            f"{user_robbing.name} stole **{amt} credits** from {user_robbed.name}. Nice!"
                        )

                    elif choice == "bigloot":
                        percentage = (
                            random.choice(list(range(5, 8)) + list(range(10, 12))) / 100
                        )
                        amt = int(percentage * user_robbing_credits)
                        await UserDatabaseFunctions.give(
                            user_giving=user_robbed, user_taking=user_robbing, amt=amt
                        ...
                        await ctx.reply(
                            f"**BLING BLING!**{user_robbing.name} stole **{amt} credits** from {user_robbed.name}. GG!"
                        )

                    elif choice == "biggestloot":
                        percentage = (
                            random.choice(list(range(8, 12)) + list(range(15, 18)))
                            / 100
                        )
                        amt = int(percentage * user_robbing_credits)
                        await UserDatabaseFunctions.give(
                        ...
                        await ctx.reply(
                            f"**WHOA YOU HIT THE JACKPOT**{user_robbing.name} stole **{amt} credits** from {user_robbed.name}. {int(percentage*100)}% of their whole credits. Just insane."
                        )

                    ImportantFunctions = self.bot.get_cog("ImportantFunctions")
                    await ImportantFunctions.update_logs(
                        type="rob",
                        user_id=user_robbed.id,
                        related_user_id=user_robbing.id,
                        amt=amt,
                        guild_id=ctx.guild.id,
                    )
                    ...
                    await ctx.reply(
                        f"{user_robbed.name} has been robbed in the past 10 minutes. Give it a rest."
                    )
                    ctx.command.reset_cooldown(ctx)
    ...
    @commands.command(
        name="Spin",
        aliases=["stw", "wheel"],
        help=f"Spin the wheel of fortune to get free karma or maybe even lose a couple of hundred credits.",
    )
    async def wheel(self, ctx):
        ...
        random_wheel_outcome = random.choices(
            wheel_outcomes, weights=(20, 15, 15, 15, 15, 10, 10), k=1
        )[0]
        ...
        if random_wheel_outcome == "Free_Credits":
            ...
            options = [
                f"You committed Tax Fraud and got {amt} credits!",
                f"The mafia decided to give you some dough. You got {amt} credits",
                f"It's your Birthday! You got {amt} credits.",
                f"Here is a GET OUT OF JAIL CARD. Collect {amt} as you go.",
            ]
            embed = discord.Embed(
                title=f"{ctx.author.name} has spinned the Wheel of Fortune",
            ...
            options = [
                f"God messed up the balance sheet and you get {amt} Karma!",
                f"You got free {amt} karma for breathing. Yay!",
                f"You got {amt} free Karma.",
            ]
            embed = discord.Embed(
                title=f"{ctx.author.name} has spinned the Wheel of Fortune",
                description=random.choice(options),
            ...
            options = [
                f"The IRS raided your house. You lost {amt} credits.",
                f"You lost {amt} credits!",
                f"You got a GO TO JAIL CARD and paid {amt} credits for bail.",
            ]
            embed = discord.Embed(
                title=f"{ctx.author.name} has spinned the Wheel of Fortune",
                description=random.choice(options),
            ...
            options = [
                f"You posted a shit meme and lost {amt} Karma.",
                f"You lost {amt} Karma for fun.",
                f"Karma? Who needs that? You lost {amt} karma.",
            ]
            embed = discord.Embed(
                title=f"{ctx.author.name} has spinned the Wheel of Fortune",
                description=random.choice(options),
            ...
            UserDatabaseFunctions = self.bot.get_cog("UserDatabaseFunctions")
            await UserDatabaseFunctions.edit_badges(
                user=user, badge_name="Double Credits Badge", action="add"
            )
            await asyncio.sleep(int(60 * 30))
            await UserDatabaseFunctions.edit_badges(
                user=user, badge_name="Double Credits Badge", action="remove"
            )

        elif random_wheel_outcome == "Karma_Boost":
            user = ctx.author
            options = [
                f"You get a Karma boost!\nAny Karma you earn in the next 30 minutes are doubled!"
            ]
            embed = discord.Embed(
                title=f"{ctx.author.name} has spinned the Wheel of Fortune!",
                description=f"{random.choice(options)}",
            ...
            UserDatabaseFunctions = self.bot.get_cog("UserDatabaseFunctions")
            await UserDatabaseFunctions.edit_badges(
                user=user, badge_name="Double Karma Badge", action="add"
            )
            await asyncio.sleep(int(60 * 30))
            await UserDatabaseFunctions.edit_badges(
                user=user, badge_name="Double Karma Badge", action="remove"
            )

        elif random_wheel_outcome == "Nothing":
            user = ctx.author
            numbers = (
                list(range(1, 200)) + list(range(400, 500)) + list(range(2000, 2050))
            )
            amt = random.choice(numbers)

            options = [
                f"You win nothing lol.",
                "You get absolutely nothing. Congrats!",
                "Here you go. Have a nothing !",
            ]
            embed = discord.Embed(
                title=f"{ctx.author.name} has spinned the Wheel of Fortune",
                description=random.choice(options),
    ...
    @commands.command(
        name="Give",
        aliases=["pay"],
        help="Give your credits to others. Boost badges are not applicable for this command",
    )
    async def give(self, ctx, user_mentioned: discord.Member, amt: str):
        ...
        if await UserDatabaseFunctions.get_user_passive_mode(user):
            ...
            await ctx.send(
                f"{user_mentioned.name} is in passive mode. You can't share with them."
            )
        ...
        if amt <= 0:
            ...
            await UserDatabaseFunctions.give(
                user_giving=user, user_taking=user_mentioned, amt=amt, boost_check=False
            )
            await ctx.reply(
                f"{user.name} gave {user_mentioned.name}, **{amt} credits**."
            )
            ImportantFunctions = self.bot.get_cog("ImportantFunctions")
            await ImportantFunctions.update_logs(
                type="give",
    ...
    @commands.group(
        name="Leaderboard",
        aliases=["lb", "lboard"],
        help=f"Shows the server leaderboard",
    ...
    @leaderboard.command(
        ...
        help=f"Shows the server leaderboard according to the credits.",
    )
    async def credits_leaderboard(self, ctx, page: int = 1, globallb=False):
        sent_msg = await ctx.reply(
            embed=discord.Embed(
                title=f"{config.loading_reaction}   Server Leaderboard",
                description=f"Fetching the server leaderboard...",
                colour=random.choice(colourlist),
            )
        )
        ...
        for entry in top:
            ...
            if user or globallb:
                top_string = (
                    top_string
                    + f"`{num}.` "
                    + f" {user.mention} • `{entry['credits']} Credits `"
                    + "\n"
                )
        ...
        embed = discord.Embed(
            title=f"Credits Leaderboard",
            description=f"{top_string}",
            colour=random.choice(colourlist),
        )
        embed.set_footer(
            icon_url=ctx.author.avatar_url,
    ...
    @leaderboard.command(
        name="Karma", help=f"Shows the server leaderboard according to the karma."
    )
        ...
        for entry in top:
            ...
            if user:
                top_string = (
                    top_string
                    + f"`{num}.` "
                    + f" {user.mention} • `{entry['karma']} Karma `"
                    + "\n"
                )
    ...
    @commands.group(
        ...
        aliases=["glb", "gleaderboard", "glboard"],
        help=f"Shows the global leaderboard",
        case_insensitive=True,
        invoke_without_command=True,
    )
    ...
    @gloablleaderboard.command(
        name="Credits",
        aliases=["credit", "creds", "cred"],
        help=f"Shows the global leaderboard according to the credits.",
    )
    async def credits_globalleaderboard(self, ctx, page: int = 1):
        sent_msg = await ctx.reply(
            embed=discord.Embed(
                title=f"{config.loading_reaction}   Global Leaderboard",
                description=f"Fetching the server leaderboard...",
                colour=random.choice(colourlist),
            )
        )
        ...
        embed = discord.Embed(
            title=f"Credits Global Leaderboard",
            description=f"{top_string}",
            colour=random.choice(colourlist),
        )
        embed.set_footer(
    ...
    @gloablleaderboard.command(
        name="Karma",
        help=f"Shows the global leaderboard according to the karma.\nFormat: `{config.default_prefixes[0]}GlobalLeaderboard karma`",
    )
    async def karma_globalleaderboard(self, ctx, page: int = 1):
        ...
        embed = discord.Embed(
            title=f"Karma Global Leaderboard",
            description=f"{top_string}",
            colour=random.choice(colourlist),
        )
        embed.set_footer(
                ...
                if offset is not None:  # if we know how many entries we want
                    if column == "credits":
                        all_rows = await connection.fetch(
                            "SELECT user_id, credits FROM info ORDER BY credits DESC LIMIT 10 OFFSET $1",
                            offset,
                        )
                    elif column == "karma":
                        all_rows = await connection.fetch(
                            "SELECT user_id, karma FROM info ORDER BY karma DESC LIMIT 10 OFFSET $1",
                            offset,
                        )
                    return all_rows
                else:
                    if column == "credits":
                        all_rows = await connection.fetch(
                            "SELECT user_id, credits FROM info ORDER BY credits DESC"
                        )
                    elif column == "karma":
                        all_rows = await connection.fetch(
                            "SELECT user_id, karma FROM info ORDER BY karma DESC"
                        )
                    return all_rows

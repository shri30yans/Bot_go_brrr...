...
class Fun(commands.Cog, name="Random", description="Random stuff"):
    ...
    @commands.command(
        ...
        help=f"Removes access to all channels for a specified time period.Time can be entered in (s|m|h|d), Default time is 10 mins.",
    )
    async def concentrate(
        self, ctx, members: commands.Greedy[discord.Member], time: str = "5m"
    ):
        pos = ["s", "m", "h", "d"]
        ...
        try:
            ...
            await ctx.send(
                f"The time can only be an integer. Please enter an integer next time."
            )
            return
        ...
        embed.add_field(
            name=f":mute: | Concentration Mode activated for {ctx.message.author}",
            value=f"**{ctx.message.author}** was put on concentration mode for **{time}**!",
        )
        embed.set_footer(

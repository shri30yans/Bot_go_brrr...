...
class StarboardFunctions(commands.Cog):
    ...
    async def post_to_starboard(self, message, channel, user, emoji, reaction_name):
        ...
        if lock:
            ...
            channel.overwrites_for(channel.guild.default_role).view_channel is False
        ) and (private_channel is False):
                ...
                if len(message.reactions) == 0:
                    ...
                    for x in message.reactions:
                        if str(x.emoji) == str(emoji):
                            ...
                            if not (self_star) and (
                                message.author in users_who_have_reacted
                            ):
            ...
            if post is None:
                ...
                if str(emoji) in sb_emoji:
                    ...
                    await self.create_new_starboard_post(
                        sb_emoji=sb_emoji,
                        message=message,
                        channel=channel,
                        starboard_channel=starboard_channel,
                        starboard_post_list=starboard_post_list,
                ...
                try:
                    StarMessage = await starboard_channel.fetch_message(
                        post["star_message_id"]
                    )
                except:
                    # In case the starboard message was deleted, this deletes it from the database
                    starboard_post_list.remove(post)
                    await self.update_starboard_key(
                        starboard_info=starboard_info,
                        starboard_info_key="starboard_posts",
                        value=starboard_post_list,
                        guild_id=guild_id,
                    )
                    return
                ...
                if str(emoji) in sb_emoji:
                    ...
                    if (
                        reaction_count < starboard_stars_required
                        and len(reactions_of_post) <= 1
                    ):
    ...
    async def create_new_starboard_post(
        ...
        # Embed
        reaction_id_string = await self.starboard_string(
            reactions_of_post=reactions_of_post, sb_emoji=sb_emoji
        )
        ...
        if (channel.is_nsfw()) is False:
            ...
            embed = discord.Embed(
                color=random.choice(colourlist),
                timestamp=message.created_at,
                description="`This post is in a NSFW channel.\nUse the Jump link to view that post here or make the Starboard channel a NSFW channel for future posts.`",
            )

        embed.set_author(
            name=message.author.name, icon_url=f"{message.author.avatar_url}"
        )
        embed.add_field(
            name="Source:",
            value=f'[Jump]({message.jump_url} "See the starred post.")',
            inline=False,
        )

        if len(message.attachments) and (
            channel.is_nsfw() is False
        ):  # basically if len !=0 or if attachments are there
            embed.set_image(url=message.attachments[0].url)

        embed.set_footer(text=f"{message.id}")

        StarMessage = await starboard_channel.send(
            content=f"{reaction_id_string}", embed=embed
        )

        # Database Actions
        starboard_post_info = {
            "root_message_id": message.id,
            "star_message_id": StarMessage.id,
            "reactions": reactions_of_post,
        }
    ...
    async def starboard_string(self, reactions_of_post, sb_emoji):
        ...
        for r in ordered_reactions_of_post:
            ...
            reaction_id_string = (
                reaction_id_string + f"{ordered_reactions_of_post[r]} {reaction_id}   "
            )
    ...
    async def update_starboard(self, starboard_info, guild_id):
        ...
        ImportantFunctions = self.bot.get_cog("ImportantFunctions")
        await ImportantFunctions.update_server_settings_key_cache(
            guild_id, key="starboard", value=starboard_info_j
        )
    ...
    async def update_starboard_key(
        self, starboard_info, starboard_info_key, value, guild_id
    ):

...
class UserDatabaseFunctions(commands.Cog):
    ...
    async def get_user_cooldown(self, user):
        async with self.bot.pool.acquire() as connection:
            async with connection.transaction():
                await self.has_account(user)
                user_account = await connection.fetchrow(
                    "SELECT cooldown FROM info WHERE user_id=$1", user.id
                )
                user_account = dict(user_account)
    ...
    async def get_user_passive_mode(self, user):
        async with self.bot.pool.acquire() as connection:
            async with connection.transaction():
                await self.has_account(user)
                user_account = await connection.fetchrow(
                    "SELECT passive FROM info WHERE user_id=$1", user.id
                )
                user_account = dict(user_account)
        ...
        async with self.bot.pool.acquire() as connection:
            async with connection.transaction():
                ...
                await connection.execute(
                    "UPDATE info SET passive = $1 WHERE user_id=$2", value, user.id
                )
    ...
    async def get_user_credits(self, user):
        async with self.bot.pool.acquire() as connection:
            async with connection.transaction():
                await self.has_account(user)
                user_account = await connection.fetchrow(
                    "SELECT credits FROM info WHERE user_id=$1", user.id
                )
                user_account = dict(user_account)
    ...
    async def get_user_karma(self, user):
        async with self.bot.pool.acquire() as connection:
            async with connection.transaction():
                await self.has_account(user)
                user_account = await connection.fetchrow(
                    "SELECT karma FROM info WHERE user_id=$1", user.id
                )
                user_account = dict(user_account)
        ...
        async with self.bot.pool.acquire() as connection:
            async with connection.transaction():
                ...
                if user_giving.bot or user_taking.bot:
                    ...
                    if boost_check:
                        amt = await self.check_for_boosts(
                            user_taking, amt, type="credits"
                        )
                    await connection.execute(
                        "UPDATE info SET credits = credits - $1 WHERE user_id=$2",
                        amt,
                        user_giving.id,
                    )
                    await connection.execute(
    ...
    async def has_account(self, user: discord.Member):
        async def create_account(user: discord.Member):
            ...
            await connection.execute(
                "INSERT INTO info (user_id,credits,karma,awards_received,awards_given,badges,cooldown) VALUES ($1,0,0,$2,$3,$4,$5,)",
                user.id,
                empty_json,
                empty_json,
                badges,
        ...
        async with self.bot.pool.acquire() as connection:
            async with connection.transaction():
                ...
                if user.bot:
                    ...
                    await connection.execute(
                        "UPDATE info SET karma = karma +$1 WHERE user_id=$2",
                        amt,
                        user.id,
                    )
    ...
    async def check_for_boosts(self, user, amt, type):
        ...
        if type == "karma":
            outcome = await self.check_if_has_badge(
                user, badge_name="Double Karma Badge"
            )

        elif type == "credits":
            outcome = await self.check_if_has_badge(
                user, badge_name="Double Credits Badge"
            )
        ...
        async with self.bot.pool.acquire() as connection:
            async with connection.transaction():
                ...
                if user_recieving.bot:
                    ...
                    user_account = await connection.fetchrow(
                        "SELECT awards_received FROM info WHERE user_id=$1",
                        user_recieving.id,
                    )
                    user_account = dict(user_account)
                    ...
                    await connection.execute(
                        "UPDATE info SET awards_received = $1 WHERE user_id=$2",
                        awards_update,
                        user_recieving.id,
                    )
                ...
                if user_giving.bot:
                    ...
                    user_account = await connection.fetchrow(
                        "SELECT * FROM info WHERE user_id=$1", user_giving.id
                    )
                    user_account = dict(user_account)
                    ...
                    await connection.execute(
                        "UPDATE info SET awards_given = $1 WHERE user_id=$2",
                        awards_update,
                        user_giving.id,
                    )
        ...
        async with self.bot.pool.acquire() as connection:
            async with connection.transaction():
                if user.bot:
                    ...
                    user_account = await connection.fetchrow(
                        "SELECT badges FROM info WHERE user_id=$1", user.id
                    )
                    user_account = dict(user_account)
                    ...
                    await connection.execute(
                        "UPDATE info SET badges = $1 WHERE user_id = $2",
                        badges_update,
                        user.id,
                    )
                    return True
            ...
            async with connection.transaction():
                if user.bot:
                    ...
                    for badge in list(badges.badges_list.values()):
                        # print(badge.name)
                        if (badge.karma_required is not None) and (
                            badge.karma_required <= user_account["karma"]
                        ):
                            await self.edit_badges(
                                user, badge_name=badge.name, action="add"
                            )

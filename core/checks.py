...
def CustomCooldown(key, delay):
    async def get_last_cooldown(ctx):
            ...
            async with ctx.bot.pool.acquire() as connection:
                async with connection.transaction():
                    ...
                    await connection.execute(
                        "UPDATE info SET cooldown = $1 WHERE user_id=$2",
                        json_datetime,
                        user.id,
                    )
        ...
        if key in cooldown_info:
            ...
            if (
                retry_after <= 0
            ):  # Delay - Time elapsed will give less than 0 if the time elapsed since last command is greater than 0
                await set_cooldown_to_current(user=user, key=key)
                return True
            else:
                raise CustomCommandOnCooldown(
                    ctx=ctx, user=ctx.author, retry_after=retry_after
                )
